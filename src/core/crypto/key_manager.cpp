...
void KeyManager::WriteKeyToFile(KeyCategory category, std::string_view keyname,
    ...
    if (category == KeyCategory::Title) {
        // Do not auto-generate title key files. Title keys should only come from explicit
        // user-provided key files (e.g. title.keys).
        return;
    }

    const auto citron_keys_dir = Common::FS::GetCitronPath(Common::FS::CitronPath::KeysDir);

    std::string filename = dev_mode ? "dev.keys_autogenerated" : "prod.keys_autogenerated";

    if (category == KeyCategory::Console) {
