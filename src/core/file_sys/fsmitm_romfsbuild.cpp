...
void RomFSBuildContext::VisitDirectory(VirtualDir romfs_dir, VirtualDir ext_dir,
                                       std::shared_ptr<RomFSBuilder::RomFSBuildDirectoryContext> parent) {
    for (auto& child_romfs_file : romfs_dir->GetFiles()) {
        const auto name = child_romfs_file->GetName();
        const auto child = std::make_shared<RomFSBuilder::RomFSBuildFileContext>();
        // Set child's path.
        ...
        ASSERT(child->path_len < RomFSBuilder::FS_MAX_PATH);
    ...
    for (auto& child_romfs_dir : romfs_dir->GetSubdirectories()) {
        ...
        const auto child = std::make_shared<RomFSBuilder::RomFSBuildDirectoryContext>();
        // Set child's path.
        child->cur_path_ofs = parent->path_len + 1;
        child->path_len = child->cur_path_ofs + static_cast<u32>(name.size());
        child->path = parent->path + "/" + name;
...
bool RomFSBuildContext::AddDirectory(std::shared_ptr<RomFSBuilder::RomFSBuildDirectoryContext> parent_dir_ctx,
                                     std::shared_ptr<RomFSBuilder::RomFSBuildDirectoryContext> dir_ctx) {
    // Add a new directory.
    num_dirs++;
    dir_table_size += sizeof(RomFSBuilder::RomFSDirectoryEntry) + Common::AlignUp(dir_ctx->path_len - dir_ctx->cur_path_ofs, 4);
...
bool RomFSBuildContext::AddFile(std::shared_ptr<RomFSBuilder::RomFSBuildDirectoryContext> parent_dir_ctx,
                                std::shared_ptr<RomFSBuilder::RomFSBuildFileContext> file_ctx) {
    // Add a new file.
    num_files++;
    file_table_size += sizeof(RomFSBuilder::RomFSFileEntry) + Common::AlignUp(file_ctx->path_len - file_ctx->cur_path_ofs, 4);
...
RomFSBuildContext::RomFSBuildContext(VirtualDir base_, VirtualDir ext_)
    ...
    root = std::make_shared<RomFSBuilder::RomFSBuildDirectoryContext>();
    root->path = "\0";
...
std::vector<std::pair<u64, VirtualFile>> RomFSBuildContext::Build() {
    const u64 dir_hash_table_entry_count = RomFSBuilder::romfs_get_hash_table_count(num_dirs);
    const u64 file_hash_table_entry_count = RomFSBuilder::romfs_get_hash_table_count(num_files);
    dir_hash_table_size = 4 * dir_hash_table_entry_count;
    file_hash_table_size = 4 * file_hash_table_entry_count;

    // Assign metadata pointers.
    RomFSBuilder::RomFSHeader header{};

    std::vector<u8> metadata(file_hash_table_size + file_table_size + dir_hash_table_size + dir_table_size);
    ...
    std::sort(directories.begin(), directories.end(), [](const auto& a, const auto& b) { return a->path < b->path; });

    // Determine file offsets.
    u32 entry_offset = 0;
    std::shared_ptr<RomFSBuilder::RomFSBuildFileContext> prev_file = nullptr;
    for (const auto& cur_file : files) {
        file_partition_size = Common::AlignUp(file_partition_size, 16);
        cur_file->offset = file_partition_size;
        file_partition_size += cur_file->size;
        cur_file->entry_offset = entry_offset;
        entry_offset += u32(sizeof(RomFSBuilder::RomFSFileEntry) + Common::AlignUp(cur_file->path_len - cur_file->cur_path_ofs, 4));
        prev_file = cur_file;
    }
    ...
    for (const auto& cur_dir : directories) {
        ...
        entry_offset += u32(sizeof(RomFSBuilder::RomFSDirectoryEntry) + Common::AlignUp(cur_dir->path_len - cur_dir->cur_path_ofs, 4));
    }
    // Assign deferred parent/sibling ownership.
    ...
    // Set header fields.
    header.header_size = sizeof(RomFSBuilder::RomFSHeader);
    ...
    header.file_partition_ofs = RomFSBuilder::ROMFS_FILEPARTITION_OFS;
    ...
    std::vector<u8> header_data(sizeof(RomFSBuilder::RomFSHeader));
    ...
    for (const auto& cur_file : files) {
        RomFSBuilder::RomFSFileEntry cur_entry{};

        cur_entry.parent = cur_file->parent->entry_offset;
        cur_entry.sibling =
            cur_file->sibling == nullptr ? RomFSBuilder::ROMFS_ENTRY_EMPTY : cur_file->sibling->entry_offset;
        cur_entry.offset = cur_file->offset;
        cur_entry.size = cur_file->size;

        const auto name_size = cur_file->path_len - cur_file->cur_path_ofs;
        const auto hash = RomFSBuilder::romfs_calc_path_hash(cur_file->parent->entry_offset, cur_file->path,
        ...
        out.emplace_back(cur_file->offset + RomFSBuilder::ROMFS_FILEPARTITION_OFS, std::move(cur_file->source));
        std::memcpy(file_table.data() + cur_file->entry_offset, &cur_entry, sizeof(RomFSBuilder::RomFSFileEntry));
        std::memset(file_table.data() + cur_file->entry_offset + sizeof(RomFSBuilder::RomFSFileEntry), 0,
                    Common::AlignUp(cur_entry.name_size, 4));
        std::memcpy(file_table.data() + cur_file->entry_offset + sizeof(RomFSBuilder::RomFSFileEntry),
                    cur_file->path.data() + cur_file->cur_path_ofs, name_size);
    }

    // Populate dir tables.
    for (const auto& cur_dir : directories) {
        RomFSBuilder::RomFSDirectoryEntry cur_entry{};

        cur_entry.parent = cur_dir == root ? 0 : cur_dir->parent->entry_offset;
        cur_entry.sibling =
            cur_dir->sibling == nullptr ? RomFSBuilder::ROMFS_ENTRY_EMPTY : cur_dir->sibling->entry_offset;
        cur_entry.child =
            cur_dir->child == nullptr ? RomFSBuilder::ROMFS_ENTRY_EMPTY : cur_dir->child->entry_offset;
        cur_entry.file = cur_dir->file == nullptr ? RomFSBuilder::ROMFS_ENTRY_EMPTY : cur_dir->file->entry_offset;

        const auto name_size = cur_dir->path_len - cur_dir->cur_path_ofs;
        const auto hash = RomFSBuilder::romfs_calc_path_hash(cur_dir == root ? 0 : cur_dir->parent->entry_offset,
        ...
        std::memcpy(dir_table.data() + cur_dir->entry_offset, &cur_entry, sizeof(RomFSBuilder::RomFSDirectoryEntry));
        std::memset(dir_table.data() + cur_dir->entry_offset + sizeof(RomFSBuilder::RomFSDirectoryEntry), 0, Common::AlignUp(cur_entry.name_size, 4));
        std::memcpy(dir_table.data() + cur_dir->entry_offset + sizeof(RomFSBuilder::RomFSDirectoryEntry), cur_dir->path.data() + cur_dir->cur_path_ofs, name_size);
    }

    // Write metadata.
    out.emplace_back(header.dir_hash_table_ofs, std::make_shared<VectorVfsFile>(std::move(metadata)));
