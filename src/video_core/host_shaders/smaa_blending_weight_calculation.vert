...
void main() {
    ...
    vec2 size = vec2(textureSize(edges_tex, 0));
    pix_coord = tex_coord * size;
    vec2 texel = 1.0 / size;
    offset[0] = vec4(tex_coord + vec2(-texel.x, 0.0), tex_coord + vec2(texel.x, 0.0));
    offset[1] = vec4(tex_coord + vec2(0.0, -texel.y), tex_coord + vec2(0.0, texel.y));
    offset[2] = vec4(0.0);
}
