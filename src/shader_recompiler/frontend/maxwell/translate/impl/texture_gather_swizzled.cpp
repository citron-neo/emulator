...
namespace {
enum class PrecisionTGS : u64 {
    F32,
    F16,
};

enum class ComponentTypeTGS : u64 {
    R = 0,
    G = 1,
    B = 2,
    A = 3,
...
union EncodingTGS {
    u64 raw;
    BitField<55, 1, PrecisionTGS> precision;
    BitField<52, 2, ComponentTypeTGS> component_type;
...
IR::Value GatherMakeOffset(TranslatorVisitor& v, IR::Reg reg) {
...
IR::Value GatherSample(TranslatorVisitor& v, u64 insn) {
    const EncodingTGS tld4s{insn};
    ...
    if (tld4s.precision == PrecisionTGS::F16) {
    ...
    if (tld4s.aoffi != 0) {
        ...
        IR::Value offset = GatherMakeOffset(v, reg_b);
        if (tld4s.dc != 0) {
...
IR::Reg GatherRegStoreComponent32(u64 insn, size_t index) {
    const EncodingTGS tlds4{insn};
    switch (index) {
    case 0:
...
void GatherStore32(TranslatorVisitor& v, u64 insn, const IR::Value& sample) {
    for (size_t component = 0; component < 4; ++component) {
        const IR::Reg dest{GatherRegStoreComponent32(insn, component)};
        v.F(dest, IR::F32{v.ir.CompositeExtract(sample, component)});
    }
}

IR::U32 GatherPack(TranslatorVisitor& v, const IR::F32& lhs, const IR::F32& rhs) {
    return v.ir.PackHalf2x16(v.ir.CompositeConstruct(lhs, rhs));
}

void GatherStore16(TranslatorVisitor& v, u64 insn, const IR::Value& sample) {
    std::array<IR::F32, 4> swizzled;
    for (size_t component = 0; component < 4; ++component) {
        swizzled[component] = IR::F32{v.ir.CompositeExtract(sample, component)};
    }
    const EncodingTGS tld4s{insn};
    v.X(tld4s.dest_reg_a, GatherPack(v, swizzled[0], swizzled[1]));
    v.X(tld4s.dest_reg_b, GatherPack(v, swizzled[2], swizzled[3]));
}
} // Anonymous namespace

void TranslatorVisitor::TLD4S(u64 insn) {
    const IR::Value sample{GatherSample(*this, insn)};
    if (EncodingTGS{insn}.precision == PrecisionTGS::F32) {
        GatherStore32(*this, insn, sample);
    } else {
        GatherStore16(*this, insn, sample);
    }
}
