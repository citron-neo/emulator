...
DiscordImpl::DiscordImpl(Core::System& system_,
    ...
    : system{system_}, play_time_manager{play_time_manager_} {
    DiscordEventHandlers handlers{};
    handlers.ready = OnDiscordReady;
    handlers.disconnected = OnDiscordDisconnected;
    handlers.errored = OnDiscordError;

    Discord_Initialize("1484148764850786414", &handlers, 1, nullptr);
...
std::string DiscordImpl::FormatTotalPlayTime(const u64 total_seconds) const {
    if (total_seconds < 60) {
        return "Total playtime: < 1m";
    }

    const u64 total_minutes = total_seconds / 60;
    const u64 hours = total_minutes / 60;
    const u64 minutes = total_minutes % 60;
    if (hours == 0) {
        return fmt::format("Total playtime: {}m", minutes);
    }
    return fmt::format("Total playtime: {}h {}m", hours, minutes);
}
...
void DiscordImpl::UpdateGameStatus(bool use_default) {
    ...
    presence.details = game_details.c_str();
...
void DiscordImpl::Update() {
    ...
    if (is_powered_on) {
        ...
        game_details = FormatTotalPlayTime(play_time_manager.GetPlayTime(program_id));

        UpdateGameStatus(program_id == 0);
    } else {
        const std::string default_text = "Citron Is A Homebrew Emulator For The Nintendo Switch";
        const std::string default_image = "citron_logo";
        DiscordRichPresence presence{};
