...
class PosterSelectionDialog : public QDialog {
    Q_OBJECT

public:
    explicit PosterSelectionDialog(QWidget* parent, u64 program_id, const QString& game_name,
                                   Citron::SteamGridDB* sgdb);
    ~PosterSelectionDialog() override;
...
private:
    ...
    void AddPosterOption(const Citron::SteamGridDBPoster& poster, int index);
    void DownloadThumbnail(const QString& url, QLabel* target_label);
    void OnPosterSelected(const std::string& url);
